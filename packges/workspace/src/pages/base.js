 ...
 * setData 封装
 */
const setData = (instance, data) => {
  if (tool.setData) {
    tool.setData(instance, data)
  } else {
 ...
 * 处理一些特殊的页面
 */
const dealWithPage = (evt, window, value) => {
  const {
    type
  , url
  } = evt
  ...
  if (value === 'webview') {
    ...
    // 补全 url
    const _url = mp.$$adapter.tool.completeURL(url, window.location.origin)
    const options = {url: `/pages/webview/index?url=${encodeURIComponent(_url)}`}
 ...
 * 处理 query 参数
 */
const dealWithShareQuery = (window, name, data) => {

  if (window && window[name]) {

    const shareOptions = { ...window[name](data) }
  ...
  , methods: {
      ...
      onLoad(query) {
        ...
        this.document.documentElement.addEventListener('$$domNodeUpdate', () => {
          if (pageConfig.rem) {
            ...
            if (!rootFontSize) {
              rootFontSize = wx.getSystemInfoSync().screenWidth / 16 + 'px'
            }
            if (rootFontSize !== this.data.rootFontSize) {
              setData(this, {rootFontSize})
            }
          }
        ...
        this.window.$$createIntersectionObserver =
          options => wx.createIntersectionObserver(this, options)

        // 处理 openerEventChannel 获取
        this.window.$$getOpenerEventChannel =
    ...
    , onReady() {
        ...
        if (this.pageConfig.loadingView) {
          setTimeout(
            () => setData(this, {loading: false})
          , 1000
          ) // 1s 后再删除，确保页面初始渲染逻辑完成
        }
